...
def find_k(docs, min_k=1, max_k=40, min_df=2):
    ...
    sorted_list = sorted(enumerate(scores), key=lambda x: x[1])
    sorted_list = reversed(sorted_list)
    for index, value in sorted_list:
        if index > math.sqrt(len(data)+1):
            return  index



topic_Num=find_k(docs=pure_mywords_no_stopwrd, min_k=1, max_k=60, min_df=2)
...
#     md1.train(i)
#     print("迭代次数：{}\tLog: {}".format(i,md1.ll_per_word))
# for k in range(md1.k):
#     print('Top 7 words of topic #f'.format(k))
#     print(md1.get_topic_words(k, top_n = 7))

mdl = tp.LDAModel(k=topic_Num, min_df=2, seed=555)

for words in pure_mywords_no_stopwrd:
    # 确认words 是 非空词语列表
    if words:
        mdl.add_doc(words)
mdl.train()
...
mywords5_no_stopwrd = [remove_stopwrd(lis) for lis in mywords5]
mywords_Pure_no_stopwrd = [remove_stopwrd(lis) for lis in mywords]
dictionary_trigram_Pure = gensim.corpora.Dictionary(mywords_Pure_no_stopwrd)
...
corpus_pure_gram = [dictionary_trigram_Pure.doc2bow(text) for text in mywords_Pure_no_stopwrd]
topN_Pure_freqs = [[] for i in range(len(corpus_pure_gram))]
i=0
...
corpus_gram= [dictionary_gram.doc2bow(text) for text in mywords_no_stopwrd]
lda = LdaModel(corpus=corpus_gram, id2word=dictionary_gram, num_topics=n_topics, passes=60)
list_arti=[]
...
d = pyLDAvis.gensim.prepare(lda, corpus_gram, dictionary_gram)
# pyLDAvis.show(d)
pyLDAvis.save_html(d, '610topic.html')
...
if replacements > 0:
    # 将修改后的内容写回HTML文件
    with open('1024topic.html', 'w') as file:
