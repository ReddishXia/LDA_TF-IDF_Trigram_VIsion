...
def prepare(topic_term_dists, doc_topic_dists, doc_lengths, vocab, term_frequency,
   ...
   """
    ...
    return drawTheCiecle.draw(x_min_value, x_max_value, y_min_value, y_max_value, dist['x'], dist['y'], dist['Freq']),ts.getFreq_Term(terms_array, freqs_array, totals_array, category_array),PreparedData(topic_coordinates, topic_info,token_table, R, lambda_step, plot_opts, client_topic_order)
