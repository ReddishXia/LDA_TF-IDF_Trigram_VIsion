...
rootPath = r"C:\Users\79988\Desktop\XMLCollection"
resultPath = r"C:\Users\79988\Desktop"
...
#############################################################
############################################################
#比如针对Method，我们可以给出这篇文章主要用了什么Method；针对results, discussion, conclusion这些sections，可以给出这篇文章主要得出了什么

docs = dict.fromkeys(xml_papersw)
docs_Method = dict.fromkeys(xml_papersw)
docs_Results = dict.fromkeys(xml_papersw)
docs_Discussion = dict.fromkeys(xml_papersw)
docs_Conclusion = dict.fromkeys(xml_papersw)
...
for k, article in enumerate(xml_papers):
    ...
    docs[xml_papersw[k]] = []
    docs_Conclusion[xml_papersw[k]] = []
    docs_Results[xml_papersw[k]] = []
    docs_Method[xml_papersw[k]] = []
    docs_Discussion[xml_papersw[k]] = []
            ...
            for inf in x.findall('infon'):
                if inf.attrib == {'key': 'section_type'}:
                    ...
                    if inf.text not in Other_sections:
                        ...
                        if inf.text in ['METHODS']:
                            docs_Method[xml_papersw[k]].append(temp1)
                        if inf.text in ['DISCUSS']:
                            docs_Discussion[xml_papersw[k]].append(temp1)
                        if inf.text in ['RESULTS']:
                            docs_Results[xml_papersw[k]].append(temp1)
                        if inf.text in ['CONCL']:
                            docs_Conclusion[xml_papersw[k]].append(temp1)
    ...
    docs_Method[xml_papersw[k]] = list(filter(None, docs_Method[xml_papersw[k]]))
    docs_Discussion[xml_papersw[k]] = list(filter(None, docs_Discussion[xml_papersw[k]]))
    docs_Results[xml_papersw[k]] = list(filter(None, docs_Results[xml_papersw[k]]))
    docs_Conclusion[xml_papersw[k]] = list(filter(None, docs_Conclusion[xml_papersw[k]]))

# list(docs.keys()).index('PMC7084952.xml')

# joining texts of each article into one string.
a=1
# def words_Process_Flow(docs,docs_Name):
print(varname(docs))
...
for i in range(len(corpus_trigram)):
    ...
    if wd==0:
        ...
        plt.savefig(os.path.join(resultPath, f'The Short Article of Trigram_figure_{xml_papersw[i][:-5]}'), bbox_inches="tight")
        plt.close()
    ...
    if wd == 1:
        ...
        # Saving the figures in result path:
        plt.savefig(os.path.join(resultPath, f'The Regular Article  of Trigram_figure_{xml_papersw[i][:-5]}'), bbox_inches="tight")
        plt.close()
# words_Process_Flow(docs_Method,"Method")
# words_Process_Flow(docs_Discussion)
# words_Process_Flow(docs_Results)
# words_Process_Flow(docs_Conclusion)