...
var LDAvis = function(to_select, data_or_file_name, color1, color2) {
    ...
    var K, // number of topics
        ...
        color1 = typeof color1 !=='undefined' ? color1: "#1f77b4",
        color3 = typeof color1 !=='undefined' ? color1: "#d62728",// baseline color for default topic circles and overall term frequencies
    ...
    function visualize(data) {
        ...
        function init_forms(topicID, lambdaID, visID) {
            ...
            inputDiv.setAttribute("style", "width: 1500px"); // to match the width of the main svg element
            ...
            rightDiv.setAttribute("style", "padding: 5px; background-color: #e8e8e8; display: inline-block; width: " + 270 + "px; height: 50px; float: right; ");
            inputDiv.appendChild(rightDiv);
            ...
            rightButtonDiv.setAttribute("style", "padding: 5px; background-color: #e8e8e8; display: inline-block; width: " + 270 + "px; height: 700px; float: right;");
            rightButtonDiv.setAttribute("id", "ArticleContext");
            inputDiv.appendChild(rightButtonDiv);
            var container = d3.select("#ArticleContext")
                .style("overflow", "auto");
            var artplot = container.append("g")
                .attr("id", "article")
            ...
            var articleZero = document.createElement("div");
            articleZero.setAttribute("style", "padding: 5px; height: 20px; width: 220px; font-family: sans-serif; float: left");
            articleZero.setAttribute("id", "mytext");
            rightDiv.appendChild(articleZero);
            var xx = d3.select("#" + "mytext")
                .append("text")
                .attr("x", 0)
                .attr("y", 0)
                .style("font-size", "14px")
                .text("Click to jump to the original text:");
        ...
        function topic_on(circle) {
            ...
            //data2取得topic我滴
            var Mydat2 = lamData.filter(function (d) {
                return d.myCategory == "Topic" + topics;
            });
            // define relevance:
            for (var i = 0; i < dat2.length; i++) {
            ...
            var Mydat3 = Mydat2;
            var pmcID = []
            var Myart = Mydat3.map(function (d) {
                return d.myArticle;
            })
            var artplot = d3.select("#article");
            if(circleTemplte!=circle.id.toString()){
                // console.log("sucess")
                circleTemplte=circle.id.toString()
                if(artplot.selectAll("*").size()==0){
            for (var i = 0; i < Myart.length; i++) {
                var articleLink = "https://pubmed.ncbi.nlm.nih.gov/" + Myart[i].replace("PMC", '');
                var aElement = artplot.append("a")
                    .style("white-space", "normal")
.attr("href", articleLink)
                    .style("display", "block")
                    .style("margin-bottom", "10px"); // Add margin between items

                aElement.append("text")

                    .text(Myart[i].replace(".xml",''))
                    .style("font-size", "16px");

                aElement.append("img")
                    .attr("src", Myart[i].replace("xml",'png'))
                    .attr("width", "200px")
                    .attr("height", "150px");
                console.log(Myart[i].replace("xml",'png'))

                // artplot.append("a")
                //     .style("white-space", "normal")
                //     // .attr("x", 1255)
                //     // .attr("y", 10+i*200)
                //     .attr("href", "https://pubmed.ncbi.nlm.nih.gov/"+Myart[i].replace("PMC",''))
                //     .append("text")
                //     .text(Myart[i])
                //     .style("font-size", "16px")
                //     .style("display", "block")
                //     ;
                //
                // artplot.append("svg:image")
                //     // .attr("xlink:href", "Regular Article Top ten n-grams-WordCloud for "+Myart[i]+".png")
            ...
            var y = d3.scaleBand()
                ...
                .rangeRound([0, barheight])
                .padding(0.1);
console.log(d3.max(dat3, function(d) {
                    return d.Total;
                }))
//console.log(dat3)
            var x = d3.scaleLinear()
            ...
            //change thr aricle
            d3.selectAll(to_select + " .article")
                .data(Mydat3)
                .attr("x", -5)
                .attr("y", 345)
                .attr("id", function(d) {
                    return (termID + d.myArticle);
                })
                .style("text-anchor", "end") // right align text - use 'middle' for center alignment
                .text(function(d) {
                    return d.myArticle;
...
//                console.log(x(d.Freq))
        ...
        function topic_off(circle) {
            ...
            var artplot = d3.select("#article");
            if(circleTemplte==circle.id.toString()){
                artplot.selectAll("a").remove();
                artplot.selectAll("image").remove();
